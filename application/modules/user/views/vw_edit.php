                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control js-example-basic-single" id="role_id" name="role_id" required>
                                ...
                                <?php foreach ($role as $key => $value) : ?>
                                    <option value="<?= $value['id_role'] ?>" <?= ($user['role_id'] == $value['id_role']) ? 'selected' : '' ?>><?= strtoupper($value['role'])  ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control js-example-basic-single" id="director_id" name="director_id">
                                ...
                                <?php foreach ($director as $key => $value) : ?>
                                    <option value="<?= $value['id_director'] ?>" <?= ($user['director_id'] == $value['id_director']) ? 'selected' : '' ?>><?= strtoupper($value['name'])  ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-check form-check-primary">
                            <label class="form-check-label">
                                <input type="checkbox" class="form-check-input" value="1" <?= ($user['is_active']) ? 'checked' : '' ?> name="is_active">
                                Aktif
